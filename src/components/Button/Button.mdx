...
If a button has the class `btn-primary-styles`, its background color will turn to the primary color defined in your theme or CSS. This is useful for highlighting primary actions.

To disable a button, add the class `disabled` to it. This will visually indicate the button is inactive and prevent user interaction.

<Canvas of={ButtonStories.Default} />

**CSS Classes Used:** `.btn`, `.btn-primary-styles`, `disabled`
...
### Processing State Activation
To activate the processing state, add the `.processing-active` class to your button. When `.processing-active` is present, the button displays animated diagonal stripes and shows the processing label.
